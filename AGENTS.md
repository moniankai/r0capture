...
下载的短剧视频将作为多模态大模型的输入素材，用于学习和拆解短剧剧本。对下载质量的核心要求：集数必须精准对应（第 N 集 = App 中的第 N 集）、全集完整无缺、给定剧名即可一键全自动下载。当前 `download_drama.py` 的主要问题是选集定位不准和 Hook 数据过期（stale_data），需要优化解决。
